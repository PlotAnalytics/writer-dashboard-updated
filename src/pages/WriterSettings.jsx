  ...
  const handleEdit = async (settingId) => {
    try {
      ...
      const response = await axios.post(
        "/api/master-editor/update-writer-setting",
        {
          id: settingId,
          skipQA: newSkipQA,
        }
      ...
      setError(
        error.response?.data?.error || "Failed to update writer setting"
      );
    } finally {
      setEditingId(null);
    }
    ...
    >
      ...
      <Box sx={{ p: 4 }}>
        ...
        {error && (
          <Alert
            severity="error"
            sx={{ mb: 3, background: "rgba(244, 67, 54, 0.1)", color: "white" }}
          >
            {error}
          </Alert>
        )}
        ...
        {success && (
          <Alert
            severity="success"
            sx={{ mb: 3, background: "rgba(76, 175, 80, 0.1)", color: "white" }}
          >
            {success}
          </Alert>
        )}
                ...
                >
                  ...
                  <TableCell sx={{ border: "none", py: 2 }}>
                    <FormControl size="small" sx={{ minWidth: 120 }}>
                      <Select
                        value={
                          selectedValues[setting.id] !== undefined
                            ? selectedValues[setting.id]
                            : setting.skip_qa
                        }
                        onChange={(e) =>
                          handleSkipQAChange(setting.id, e.target.value)
                        }
                        sx={{
                          color: "white",
